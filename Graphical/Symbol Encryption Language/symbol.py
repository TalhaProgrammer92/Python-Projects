        ...
        if len(self.__tokens) > 0:
            for token in self.__tokens:
                ...
                match token.symbol:
                    ...
                    case '&':
                        value: int = self.__stacks.stacks[stack_pointer].cells[
                            cell_pointer + 1].value if cell_pointer < 7 else self.__stacks.stacks[stack_pointer].cells[
                            cell_pointer - 1].value
...
class Editor(tk.Tk):
    ...
    def __save_file(self) -> None:
        ...
        if self.__source_file is None:
            ...
            self.__source_file = tfdg.asksaveasfilename(initialfile='.sym', defaultextension='.sym',
                                                        filetypes=[('Text Documents', '*.sym')])
    ...
    def __ascii_table(self) -> None:
        ...
        if os.path.exists('ascii.png'):
            ...
            tmsg.showerror('Error',
                           'The file \'ascii.png\' does not exist in current path. Please make sure you have the same file.')

    def __about(self) -> None:
        """ To open about page """
        tmsg.showinfo('About',
                      'Symbol (Esolang) IDE created by Talha Ahmad\nDated: December 2024 CE\nAll Rights are Reserved (c)')
